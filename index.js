...
class Game extends React.Component {
	...
	daily() {
		...
		this.setState({
			energy: Math.max(
				energy + (((1 * fact_avg) / 2) * workers) / 4 - workers / 16,
				0
			),
			minerals: Math.max(
				minerals + (((1 * fact_avg) / 3) * workers) / 2,
				0
			),
			food: Math.max(food + (1 * fact_avg) / 2 - workers / 5, 0),
			alloys: Math.max(
				alloys +
					(((((1 * fact_avg) / 8) * workers) / 16) * minerals) / 4,
				0
			),
			factories: factories,
			workers: tem_workers,
	...
	build_factory(idd) {
		...
		if (t.energy < 2 || t.minerals < 4 || t.workers < 1 || t.alloys < 3) {
			this.setState({
				message: "Not enough resources to build a factory"
			});
		} else {
	...
	sell_factory(idd) {
		...
		if (t.factories < 2) {
			this.setState({
				message: "Not enough resources to sell a factory"
			});
		} else {
			let new_board = JSON.parse(JSON.stringify(this.state.board));
			new_board[idd] = this.state.board[idd] - 1;
	...
	sell_food() {
		...
		if (t.food < 1) {
			this.setState({
				message: "Not enough food to meet minimum sell amount"
			});
		} else {
			this.setState({
				energy: t.energy + 0.5,
	...
	hide_div() {
		this.setState({info_display: !this.state.info_display})
	...
	render_square(idd) {
		return (
			<Square
				...
				fx={[
					() => this.build_factory(idd),
					() => this.sell_factory(idd)
				]}
			/>
		);
	}
	...
	render() {
		...
		let inf = info_display ? "info" : "info_hide";
		...
		return (
			<div className="master">
				...
				<div className = {inf} onClick = {() => this.hide_div()}>
				
				
				<p>The goal of the game is to build as many factories as possible in the least amount of turns
				 Resources are interelated. On the end of a turn, the following calculations
				are performed. Beware, when food supply hits zero, you will lose a worker</p>
				<div className = 'resource-relations'>
				<p>energy = energy + (((1 * fact_avg) / 2) * workers) / 4 - workers / 16</p>
				<p>minerals = minerals + (((1 * fact_avg) / 3) * workers) / 2</p>
				<p>food = food + (1 * fact_avg) / 2 - workers / 5</p>
				<p>alloys = alloys + (((((1 * fact_avg) / 8) * workers) / 16) * minerals) / 4</p>
				</div>
				<p className = 'thicker'>to build a factory requires: 2 energy, 4 minerals, 1 worker, and 3 alloys</p>
				<p className = 'thicker'>to hire a worker requires: 1/2 energy </p>
				</div>
				<div className="main_display">
					<ResourceDisplay
						resourceName={"message"}
						passedvalue1={message}
					/>
					<ResourceDisplay
						resourceName={"energy"}
						passedvalue1={energy}
					...
					<div className="mr_block">
						...
						<ResourceDisplay
							...
							passedMethod={() => this.rdcw()}
							counts={workers}
						/>
					</div>
					<ResourceDisplay resourceName={"day"} passedvalue1={day} />
					<p className = 'factory-map-title'>Factory map</p>
				</div>
				<div className="board">
